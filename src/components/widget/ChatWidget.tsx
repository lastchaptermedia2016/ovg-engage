  ...
  const handleOpen = useCallback(() => {
    ...
    if (!hasConsent) {
      ...
      if (messages.length === 0) {
        setIsTyping(true);
        // Trigger voice immediately on user click to ensure browser allows it
        speak(greetText);
        setTimeout(() => {
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {showPeek && !isOpen && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 z-50 max-w-xs rounded-2xl border-2 border-[#E91E63] bg-transparent p-4 shadow-xl"
          >
            <button
              onClick={() => setShowPeek(false)}
              className="absolute right-2 top-2 text-[#C2185B] hover:text-[#E91E63]"
            >
              <X className="h-3 w-3" />
            </button>
            <p className="pr-4 text-sm text-[#C2185B]">
              Hey gorgeous! ✨ Curious about our treatments? Claim{" "}
              <span className="font-semibold text-[#E91E63]">20% off</span> your
              first consultation today.
            </p>
            <button
              onClick={handleOpen}
              className="mt-2 text-sm font-medium text-[#E91E63] hover:underline"
            >
              Chat with us →
            </button>
          </motion.div>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 z-50 flex w-[380px] max-w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-2xl border-2 border-[#E91E63] bg-transparent backdrop-blur-xl shadow-2xl max-h-[min(600px,calc(100dvh-8rem))] md:w-[400px]"
          >
            {/* Header */}
            <div
            ...
            >
              <div className="flex items-center gap-3">
                <div className="flex h-9 w-9 items-center justify-center rounded-full bg-primary-foreground/20">
                  <img src="/images/luxemedspa.svg" alt="Luxe Med Spa" className="h-8 w-8 object-contain" />
                </div>
                <div>
            ...
            {/* Messages */}
            <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-black/30 backdrop-blur-sm">
              {messages.map((msg) => (
                <motion.div
                  key={msg.id}
                  initial={{ opacity: 0, y: 8 }}
            ...
            <div className="border-t border-[#E91E63]/30 px-4 py-3 bg-black/30 backdrop-blur-sm">
              <div className="flex items-center gap-2">
                <div
      ...
      <motion.button
        ...
        className="fixed bottom-6 right-6 z-50 flex h-14 w-14 items-center justify-center rounded-full text-white shadow-lg animate-pulse-glow"
        style={{ backgroundColor: "hsl(340 65% 55%)" }}
