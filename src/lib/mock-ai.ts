...
export async function generateMockAIResponse(userMessage: string, conversationHistory: ChatMessage[]): Promise<string> {
  ...
  const messages = [
    {
      role: "system",
      content: "You are a friendly, professional Luxe Med Spa concierge in New Haven. Be warm, helpful, sales-oriented, and focused on beauty/wellness treatments, bookings, pricing, and 20% off first consultation offer. Use emojis sparingly. Keep replies concise but engaging."
    },
  ...
  try {
    ...
    return data.reply || "Hmm... something went wrong. Try again?";
  } catch (err) {
    console.error("Real Grok failed:", err);

    // Your original scripted fallback
